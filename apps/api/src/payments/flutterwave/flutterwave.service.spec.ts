...
describe("FlutterwaveService.verifyWebhookSignature", () => {
  ...
  const makeService = () => {
    const configService = {
      get: jest.fn((key: string) => (key === "FLW_WEBHOOK_HASH" ? webhookSecret : "")),
    } as any;
    return new FlutterwaveService(configService);
  };

  it("accepts a correctly signed raw body", () => {
    const service = makeService();
    const rawBody = JSON.stringify({ event: "charge.completed", data: { tx_ref: "abc" } });
  ...
  it("rejects a tampered body", () => {
    ...
    const rawBody = JSON.stringify({ event: "charge.completed", data: { tx_ref: "abc" } });
    const signature = crypto.createHmac("sha256", webhookSecret).update(rawBody).digest("base64");
    const tamperedBody = JSON.stringify({ event: "charge.completed", data: { tx_ref: "xyz" } });
